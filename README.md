...
- A clear introduction leads to a page directory. Section labels form a quiet left column on larger screens; content stacks in reading order on smaller screens.
- The résumé holds the full career history; the landing page offers focus areas and a few results with organization and context.
...
3. Add an entry to the landing page’s `page-list` under **Explore**, using the existing résumé entry as the pattern. Link to the new filename and supply a short description. This directory grows vertically as you add pages, without changing the layout or adding every page to the header. Keep external destinations labeled as external, as with GitHub.
